...
df_train = df_train.sample(frac=1,random_state=0506)

# 使用data augmentation之后的数据
# q1 = q2, q2 = q3则q1 = q3之后的label = 1的数据, 及label = 0的数据
df_ext_train = pd.read_csv("../data/augmentation_results.csv", )
df_ext_train = df_ext_train.sample(frac=1).reset_index(drop=True)

# data augmentation之后的数据为三者的concatenation
df_train = pd.concat([df_train,df_ext_train])
...
if sys.argv[1] == "-predict":
	# test_x1和test_x2相似，那么test_x2和test_x1相似。 取均值。 模型不对称
